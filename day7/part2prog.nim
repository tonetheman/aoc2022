    ...
    if cdata[1] == "cd":
        if cdata[2] == "/":
            ...
            if current_dir.parent == nil:
                ...
                echo("DBG: switched to parent directory")
        else:
            for e in current_dir.entries.mitems:
                if e.name == cdata[2]:
                    current_dir = addr(e)
                    echo("DBG: switched directory to: ",cdata[2])
                    return

    elif cdata[1] == "ls":
        state = stLs
        echo("DBG: parse_command: switched state to stLs")
...
for line in data:
    ...
    if state == stLs:
        ...
        if cdata[0] == "dir":
            ...
            let name = cdata[1]
            echo("DBG: adding a real file: ", name)
...
proc part2() = 
    var total_filesystem_space = 70000000
    var needed_space_to_update = 30000000
    var unused_space = 0

    var size_of_smallest = 99999999999

    proc part2(pp : ptr SantaFile) =
        var toosmall = false
        if unused_space+pp.size < needed_space_to_update:
            toosmall = true

        echo(pp.name, " ", pp.size, " toosmall: ", toosmall)
        if toosmall == false:
            if size_of_smallest > pp.size:
                size_of_smallest = pp.size
                echo("setting smallest: ", pp.name, " ", pp.size)
    ...
    # calc unused
    unused_space = total_filesystem_space - root.size
    echo("unused space is ",unused_space)
    if unused_space < needed_space_to_update:
        echo("unused not enough")
    part2(addr root)
    echo("size of smallest: ", size_of_smallest)
