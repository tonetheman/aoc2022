...
proc p1samplePart2(inputfile: string) =
    var breakpointhit = false
        ...
        if breakpointhit:
            ...
            while num_to_move > 0:
                if num_to_move == 1:
                    ...
                    # make a tmp stack
                    var tmp : seq[char]
                    # need to keep order so cheat
                    var uselater = num_to_move
                    # move to tmp stack reversed
                    while num_to_move > 0:
                        let val = stacks[src].pop()
                        tmp.add(val)
                        num_to_move -= 1
                    # move to dest not reversed
                    while uselater > 0:
                        let val = tmp.pop()
                        stacks[dst].add(val)
                        uselater -= 1
...
# p1samplePart2("sample.txt")

proc p1part2(inputfile: string) =
    var breakpointhit = false
